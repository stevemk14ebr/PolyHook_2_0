...
bool PLH::MemAccessor::safe_mem_read(uint64_t dest, uint64_t src, uint64_t size, size_t& read) const noexcept {
	read = 0;
	return ReadProcessMemory(GetCurrentProcess(), (char*)dest, (char*)src, (SIZE_T)size, (PSIZE_T)&read) || (GetLastError() == ERROR_PARTIAL_COPY);
}
