...
#ifdef _MSC_VER
    auto retAddress = (uint32_t) _ReturnAddress();
#elif __GNUC__
    auto retAddress = (uint32_t)__builtin_return_address(0);
#else
...
TEST_CASE("ILCallback Return and Argument Re-Writing", "[ILCallback]") {
    ...
    SECTION("int, float, double, float return, std") {
        uint64_t JIT = callback.getJitFunc(
            "float", {"int", "float", "double", "int"}, arch, &myThirdCallback, "stdcall"
        );
        REQUIRE(JIT != 0);
